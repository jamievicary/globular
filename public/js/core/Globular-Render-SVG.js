...
function globular_render_0d(container, diagram, subdiagram) {
    ...
    $(container)[0].bounds = { left: -0.5, right: 0.5, top: 0.5, bottom: -0.5 };
    var circle = document.createElementNS("http://www.w3.org/2000/svg", "circle");
    circle.setAttributeNS(null, "cx", 0);
    circle.setAttributeNS(null, "cy", 0);
    circle.setAttributeNS(null, "r", circle_radius);
    var lastpoint = diagram.getLastPoint();
    circle.setAttributeNS(null, "fill", lastpoint.type.getColour(diagram.n) /*gProject.getColour(lastpoint)*/);
...
function globular_render_1d(container, diagram, subdiagram) {
    ...
    $(container)[0].bounds = { left: 0, right: length, top: 0.5, bottom: -0.5 };
    var data = { vertices: [], edges: [], dimension: 1 };
    ...
    for (let i = 0; i < diagram.data.length; i++) {
        ...
        let type = diagram.getActionType(i);
    ...
    if (subdiagram != undefined) {
        if (subdiagram.boundaryType == 's' && subdiagram.visibleBoundaryDepth == 1) {
            d.g.appendChild(SVG_create_path({
                string: SVG_move_to({ x: 0, y: 0 }) + SVG_line_to({ x: .25, y: 0 }),
            ...
            d.g.appendChild(SVG_create_path({
                string: SVG_move_to({ x: length, y: 0 }) + SVG_line_to({ x: length - .25, y: 0 }),
                stroke: highlight_colour,
                'stroke-opacity': highlight_opacity
            }));
        }
...
function globular_render_2d(container, diagram, subdiagram) {
    ...
    if ((diagram.data.length == 0) && (diagram.source.data.length == 0)) {
        var d = prepare_SVG_container(container, diagram, -0.5, 0.5, -0.5, 0.5);
        d.g.appendChild(SVG_create_path({ string: "M -0.5 -0.5 L 0.5 -0.5 L 0.5 0.5 L -0.5  0.5", fill: diagram.getLastColour(), element_type: 'region' }));
        $(container).append(d.svg);
        $(container)[0].bounds = { left: -0.5, right: 0.5, top: 0.5, bottom: -0.5 };
        return { dimension: 2, edges: [], vertices: [], edges_at_level: [[], []] };
    }
    ...
    var y_center = Math.max(1, diagram.data.length) / 2;
    var w = big_background ? 20 : data.max_x + 1;
    var h = big_background ? 20 : Math.max(1, diagram.data.length);
    ...
    let lastColour = diagram.source.source.getActionType(0).getColour(diagram.n - 2);
    ...
    */
    ...
    for (var i = 0; i < data.edges.length; i++) {
        ...
        if (start_boundary) {
            ...
            path_s += SVG_move_to({ x: edge.x, y: 0 });
        } else {
        ...
        if (drawn_something) {
            ...
            path.setAttributeNS(null, "stroke", edge.colour /*gProject.getColour(edge.type)*/);
    ...
    if (i.draw_up) {
        ...
        if (vertex == null) {
            ...
            if (next_edge == null) {
                ...
                i.path_string += SVG_line_to({ x: data.max_x + 0.5, y: Math.max(1, data.max_y) });
                i.path_string += SVG_line_to({ x: data.max_x + 0.5, y: 0 });
                var source_edges = data.edges_at_level[0];
                if (source_edges.length == 0) {
                    i.path_string += SVG_line_to({ x: -0.5, y: 0 });
                    i.path_string += SVG_line_to({ x: -0.5, y: Math.max(1, data.max_y) });
...
function top_of_edge(data, edge) {
    if (edge.finish_vertex == null) return { x: edge.x, y: Math.max(1, data.max_y) };
...
function SVG_draw_edge_section(data, edge, h_start, h_end) {
    var diagram_height = Math.max(1, data.max_y);
    ...
    if (h_start < h_end) {
        ...
        if (top_section) {
            ...
            if (edge.finish_vertex == null) {
                ...
                var vertex = edge.finish_vertex;
                if (vertex.intersection == undefined) {
                    path += SVG_bezier_to({ c1x: edge.x, c1y: vertex.y, c2x: edge.x, c2y: vertex.y - 0.4, x: edge.x, y: edge.finish_height - 0.5 });
                } else {
                    var i = vertex.intersection;
                    var left = (edge == vertex.source_edges[0]);
                    path += SVG_bezier_to({ c1x: (left ? i.bl.P3[0] : i.br.P3[0]), c1y: (left ? i.bl.P3[1] : i.br.P3[1]), c2x: (left ? i.bl.P2[0] : i.br.P2[0]), c2y: (left ? i.bl.P2[1] : i.br.P2[1]), x: edge.x, y: edge.finish_height - 0.5 });
                }
            }
        }
        if (middle_section) {
        ...
        if (bottom_section) {
            ...
            if (edge.start_vertex == null) {
                ...
                if (vertex.intersection == undefined) {
                    ...
                    var i = vertex.intersection;
                    var left = (edge == vertex.target_edges[0]);
                    path += SVG_bezier_to({ c1x: (left ? i.tl.P2[0] : i.tr.P2[0]), c1y: (left ? i.tl.P2[1] : i.tr.P2[1]), c2x: (left ? i.tl.P3[0] : i.tr.P3[0]), c2y: (left ? i.tl.P3[1] : i.tr.P3[1]), x: vertex.x, y: vertex.y });
                }
            }
        }
    }
...
function SVG_prepare(diagram, subdiagram) {
    ...
    for (var level = 0; level < diagram.source.data.length; level++) {
        ...
        let type = diagram.source.getActionType(level);
        let colour = type.getColour(diagram.n - 1);
    ...
    for (var level = 0; level < diagram.data.length; level++) {
        ...
        for (let i = new_vertices.length - 1; i >= 0; i--) {
            ...
            for (let j = vertex.backward_insert.first; j < vertex.backward_insert.last; j++) {
                let index = j;
                let structure = 'edge';
                let type = next_slice.getActionType(j);
                let colour = type.getColour(diagram.n - 1);
                ...
                var new_edge = { structure, type, colour, start_height, finish_height, x, start_vertex, finish_vertex };
                edges.push(new_edge);
    ...
    /* CONSTRAINTS
     ...
     - Vertices are at the mean input coordinate, equal to the mean output coordinate, where these exists
    */
    while (true) {
        let problem = false;
        ...
        for (var i = 0; i < vertices.length; i++) {
            ...
            */
            ...
            let source_mean = vertex.source_edges.length == 0 ? null : (vertex.source_edges[0].x + vertex.source_edges.last().x) / 2;
            let target_mean = vertex.target_edges.length == 0 ? null : (vertex.target_edges[0].x + vertex.target_edges.last().x) / 2;
            let shift_source = 0;
            let shift_target = 0;
            let shift_vertex = 0;
            if (source_mean == null) {
            ...
            if (vertex.source_edges.length == 0 && vertex.target_edges.length == 0) continue;
            
            let source_mean = vertex.source_edges.length == 0 ? vertex.x : (vertex.source_edges[0].x + vertex.source_edges.last().x) / 2;
            let target_mean = vertex.target_edges.length == 0 ? vertex.x : (vertex.target_edges[0].x + vertex.target_edges.last().x) / 2;
