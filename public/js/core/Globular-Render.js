...
function download_SVG_as_PNG(svgElement, coords, filename, svgURL, continuation) {
    if (svgURL == undefined) svgURL = new XMLSerializer().serializeToString(svgElement);
    var img = new Image();
    img.onload = function() {
        var canvas = $('<canvas>')[0];
