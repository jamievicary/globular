...
Diagram.prototype.getTarget.IntLN = function(type, key) {
    ...
    if (type.tail('L-N')) {
        ...
        var complimentary_cell = new NCell({
            id: 'Int',
            key: [cell.key.last() - s1 * s2 - 1]
        });
        ...
        target = pullthrough_top.concat(x_ings_one.concat(pullthrough_bottom.concat(x_ings_two))).concat([new NCell({
            id: 'IntI0',
            key: [cell.key.last() + s1 * s2 + 1]
        })]);
...
Diagram.prototype.interpretDrag.IntLN = function(drag) {
    ...
    var options = this.getDragOptions(up ? ['Int-L-NI0', 'Int-R-NI0', 'Int-LI0-NI0', 'Int-RI0-NI0', 'IntI0-L-NI0', 'IntI0-R-NI0',
                                            'IntI0-LI0-NI0', 'IntI0-RI0-NI0'] : ['Int-L-N', 'Int-R-N', 'Int-LI0-N', 'Int-RI0-N', 'IntI0-L-N', 'IntI0-R-N',
...
Diagram.prototype.interchangerAllowed.IntLN = function(type, key) {
    ...
    if (type.tail('L-N')) {
        x = cell.key.last() - s1 * s2 - 1;
        if (x < 0) return false;
        index = key.last() - s2 - this.crossings(s2, t1) - this.crossings(s2, s1) - t1;
        ...
        x = cell.key.last() - s1 * s2;
        if (x < 0) return false;
        index = key.last() - s1 - this.crossings(t2, s1) - this.crossings(s2, s1) - t2;
        if (index < 0 || index > this.cells.length) return false;
        expansion_base = this.getSlice(index).copy();
        ...
        x = cell.key.last() - 1;
        if (x < 0) return false;
        index = key.last() - s2 - this.crossings(t1, s2) - this.crossings(t1, t2) - t1;
        if (index < 0 || index > this.cells.length) return false;
        expansion_base = this.getSlice(index).copy();
        ...
        x = cell.key.last();
        if (x < 0) return false;
        index = key.last() - t2 - this.crossings(s1, t2) - this.crossings(t1, t2) - s1;
        if (index < 0 || index > this.cells.length) return false;
        expansion_base = this.getSlice(index).copy();
    ...
    return this.subinstructions(key, {
        list: source,
        key: source_key
    });
...
Diagram.prototype.getInverseKey.IntLN = function(type, key) {
    ...
    if (type.tail('L-N')) {
        return [key.last() - t1 - s2 - this.crossings(s2, t1) - this.crossings(s2, s1)]
    } else if (type.tail('L-NI0')) {
        return [key.last() + t2 + s1 + this.crossings(s1, t2) + this.crossings(s1, s2)]
    } else if (type.tail('R-N')) {
        return [key.last() - t2 - s1 - this.crossings(t2, s1) - this.crossings(s2, s1)]
    } else if (type.tail('R-NI0')) {
        return [key.last() + t1 + s2 + this.crossings(t1, s2) + this.crossings(s1, s2)]
    } else if (type.tail('LI0-N')) {
        return [key.last() - s2 - t1 - this.crossings(t1, s2) - this.crossings(t1, t2)]
    } else if (type.tail('LI0-NI0')) {
        return [key.last() + s1 + t2 + this.crossings(t2, s1) + this.crossings(t2, t1)]
    } else if (type.tail('RI0-N')) {
        return [key.last() - s1 - t2 - this.crossings(s1, t2) - this.crossings(t1, t2)]
    } else if (type.tail('RI0-NI0')) {
        return [key.last() + s2 + t1 + this.crossings(s2, t1) + this.crossings(t2, t1)]
    }
