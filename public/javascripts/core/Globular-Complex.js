...
Complex.prototype.complex_0 = function(diagram) {
    ...
    this.points = [];
    this.dimension = 0;
    this.min_x = -diagram.cells.length / 2;
    this.max_x = diagram.cells.length / 2;
    this.min_y = -0.5;
    this.max_y = 0.5;
    this.size = diagram.getFullDimensions();
    this.points = [{
        type: diagram.cells[0].id,
        point: [0, 0],
        logical: []
    }];
    return this;
...
Complex.prototype.complex_1 = function(diagram) {
    ...
    if (diagram.cells.length == 0) {
        ...
        this.max_y = 0.5;
        this.lines = [{
            type: diagram.source.cells[0].id,
            points: [
                [-0.5, 0],
                [0.5, 0]
            ],
    ...
    for (var level = 0; level < diagram.cells.length; level++) {
        ...
        var generator = diagram.cells[level];
        ...
        this.lines.push({
            type: rewrite.source.cells[0].id,
            points: [
                [pos, 0],
                [pos + 0.5, 0]
            ],
        ...
        first = false;
        if (level == diagram.cells.length - 1) last = true;
        this.lines.push({
            type: rewrite.target.cells[0].id,
            points: [
                [pos + 0.5, 0],
                [pos + 1, 0]
            ],
...
Complex.prototype.complex_2 = function(diagram) {
    ...
    this.lines = [];
    this.points = [];
    this.height = diagram.cells.length;
    this.dimension = 2;
    this.min_x = Number.MAX_VALUE;
    this.max_x = -Number.MAX_VALUE;
    this.min_y = 0;
    this.max_y = Math.max(1, diagram.cells.length);
    this.size = diagram.getFullDimensions();
    ...
    if (diagram.cells.length == 0) {
        ...
        if (diagram.source.cells.length == 0) {
            var tl = [-0.5, 0.5];
            var tr = [0.5, 0.5];
            var bl = [-0.5, -0.5];
            var br = [0.5, -0.5];
            var id = diagram.source.source.cells[0].id;
        ...
        this.build_layer(current_platform, null, 0, diagram.source.cells.length);
        return this;
    }

    // Find width of the widest timeslice
    var max_width = 1;
    for (height = 0; height <= diagram.cells.length; height++) {
        var layer_max_width = this.size[height];
        if (height < diagram.cells.length) {
            var rewrite = gProject.signature.getGenerator(diagram.cells[height].id);
            if ((rewrite.source.cells.length == 0) && (rewrite.target.cells.length == 0)) {
                layer_max_width += 1
            }
        }
    ...
    for (var height = 0; height < diagram.cells.length; height++) {
        ...
        var generator = diagram.cells[height];
...
Complex.prototype.build_layer = function(current_platform, generator, height, max_width) {
    ...
    var num_blocks = (generator == null ? current_platform.cells.length : current_platform.cells.length - rewrite.source.cells.length + 1);
    ...
    if (rewrite == null) {
        ...
        generator_mid_width = Math.max(0.5, (rewrite.source.cells.length + rewrite.target.cells.length) / 2);
    }
    var mid_width = num_blocks - 1 + generator_mid_width;
    var top_width = (generator == null ? base_width : current_platform.cells.length - rewrite.source.cells.length + rewrite.target.cells.length);
    ...
    */
    ...
    for (var block = 0; block < num_blocks; block++) {
        ...
        if (block == attach) {
            ...
            // Source triangles
            for (var i = 0; i < rewrite.source.cells.length; i++) {
                var source_generator = rewrite.source.cells[i];
                var b1 = [base_position + i, height];
                var b2 = [base_position + i + 0.5, height];
                var b3 = [base_position + i + 1, height];
                this.triangles.push({
                    type: gProject.signature.getGenerator(source_generator.id).source.cells[0].id,
                    points: [anchor, b1, b2],
                    logical: [height + 0.25, source_index + i + 0.25],
                    origin: "generator source left triangle " + i
                });
                this.triangles.push({
                    type: gProject.signature.getGenerator(source_generator.id).target.cells[0].id,
                    points: [anchor, b2, b3],
            ...
            // Target triangles
            for (var i = 0; i < rewrite.target.cells.length; i++) {
                var target_generator = rewrite.target.cells[i];
                var b1 = [top_position + i, height + 1];
                var b2 = [top_position + i + 0.5, height + 1];
                var b3 = [top_position + i + 1, height + 1];
                this.triangles.push({
                    type: gProject.signature.getGenerator(target_generator.id).source.cells[0].id,
                    points: [anchor, b1, b2],
                    logical: [height + 0.75, source_index + i + 0.25],
                    origin: "generator target left triangle " + i
                });
                this.triangles.push({
                    type: gProject.signature.getGenerator(target_generator.id).target.cells[0].id,
                    points: [anchor, b2, b3],
            ...
            var top_logical = [height + 0.75, source_index + (rewrite.target.cells.length == 0 ? -0.1 : 0.25)];
            var bottom_logical = [height + 0.25, source_index + (rewrite.source.cells.length == 0 ? -0.1 : 0.25)];
            this.triangles.push({
                type: left_type,
                points: [anchor, ml, tl],
                logical: top_logical,
            ...
            br = [base_position + rewrite.source.cells.length, height];
            mr = [mid_position + generator_mid_width, height + 0.5];
            tr = [top_position + rewrite.target.cells.length, height + 1];
            var right_type = rewrite.source.getTargetBoundary().cells[0].id;
            top_logical = [height + 0.75, source_index + rewrite.target.cells.length + (rewrite.target.cells.length == 0 ? 0.1 : -0.25)];
            bottom_logical = [height + 0.25, source_index + rewrite.source.cells.length + (rewrite.source.cells.length == 0 ? 0.1 : -0.25)];
            this.triangles.push({
                type: right_type,
                points: [anchor, mr, tr],
                logical: top_logical,
            ...
            source_index += rewrite.source.cells.length;
            target_index += rewrite.target.cells.length;
            base_position += rewrite.source.cells.length;
            mid_position += generator_mid_width;
            top_position += rewrite.target.cells.length;
        }
        else {
            ...
            var left_type = wire_rewrite.source.cells[0].id;
            var right_type = wire_rewrite.target.cells[0].id;
