...
exports.register_user = function(req, res) {
	...
	if (errors === "") {
		...
		var jsonFileStr = JSON.stringify({
		...
		fs.readFile('database/users/' + email + "/data.json", 'utf-8', function(err, data) {
			...
			fs.writeFile("database/users/" + email + "/data.json", data, function(err) {
				...
				var emailData = {
					from: mailGunFrom,
					to: email,
					subject: 'New password for globular.science',
					html: "Hello,<br><nr>Your new password for Globular is: <b>" + new_pass + "</b><br><br>Please click <a href = '" + cclink + "'>here</a> to activate it.<br><br>We suggest you log in now and change it to something easier to remember."
				}
				mailgun.messages().send(emailData, function(err, body) {
					if (err) {
						...
						res.send({
							success: true,
							msg: "New password sent, please check your inbox."
						});
					}
				});
			});
